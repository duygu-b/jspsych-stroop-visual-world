...
var jsPsychFreeSort = (function (jspsych) {
  ...
  function definite_coordinate(coords, idx) {
    return { x: coords[idx].x, y: coords[idx].y };
  }
  ...
  const info = {
    ...
    parameters: {
	  ...
	  /* Added anew parameter to have the option to assign defined positions rather than being limited to randomized ones */
        start_positions: {
        type: jspsych.ParameterType.COMPLEX,
        array: true,
        default: null,
        nested: { x: { type: jspsych.ParameterType.INT }, y: { type: jspsych.ParameterType.INT } },
      }
    },
    data: {
  ...
  class FreeSortPlugin {
    ...
    trial(display_element, trial) {
      ...
      if (trial.sort_area_shape == "square") {
      ...
      for (let i = 0; i < stimuli.length; i++) {
		...
		if (trial.start_positions) { /* happy little new option here */                              
		coords = definite_coordinate(trial.start_positions, i);
		} else if (trial.stim_starts_inside) {
